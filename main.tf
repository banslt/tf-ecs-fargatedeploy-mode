...
module "fargate-deploy" {
  source          = "./modules/fargate-deploy"
}

module "svc-scaling" {
  source          = "./modules/svc-scaling"
  cluster_name    = module.fargate-deploy.aws_ecs_cluster_name
  service_name    = module.fargate-deploy.aws_ecs_service_name
  alarm_name      = "ba_cpu_stressapp"
  scale_policy_name_prefix = "ba_cpu_stressapp"
  ecs-autoscale-role_arn = module.fargate-deploy.aws_iam_role_ecs_asg_role_arn
  statistic       = "Average"
  scale_up_adjustment = "100"
  scale_down_adjustment = "-50"
  threshold_up = "90"
  threshold_down = "30"
  dims = [ {name= "ClusterName",value=module.fargate-deploy.aws_ecs_cluster_name }
          ,{name= "ServiceName",value=module.fargate-deploy.aws_ecs_service_name }
         ]
}
...
module "svc-scaling-lb-rsp-time" {
  ...
  threshold_down  = "0"
  ecs-autoscale-role_arn = module.fargate-deploy.aws_iam_role_ecs_asg_role_arn
  dims = [ {name= "TargetGroup" ,value=module.fargate-deploy.aws_alb_target_group_app_arn_suffix }
          ,{name= "LoadBalancer",value=module.fargate-deploy.aws_alb_arn_suffix }
         ]
  }
